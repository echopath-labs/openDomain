...
- `opendomain/` 是 OpenDomain 自己的真实业务模型。
...
CLI 不传 source path 时，会把当前工作目录视为项目根目录，并按以下规则解析：

1. canonical `opendomain/` 存在时只读取它；
2. canonical 不存在而 legacy `domain/` 存在时，在 `0.x` 期间兼容读取并给出 warning；
3. 两个根目录同时存在时，canonical 胜出并给出 warning，不合并两个语料库；
4. 两者都不存在，或选中的工作区没有可读取的语义 Markdown 时，命令失败；
5. 默认只扫描 `contexts/`、`concepts/`、`rules/`、`lifecycles/`、`events/` 和
   `candidates/`，不会扫描 `examples/`、`generated/` 或 integration 配置。

显式传入 Markdown 文件或目录时，CLI 只处理该目标。这适合验证示例、fixture
或项目外的语料；外部路径不会被隐式发现。隐式工作区必须位于当前项目内，
symlink descendant 不会进入默认语料。

默认 index 路径跟随工作区模式：

- canonical：`opendomain/generated/index.json`
- legacy 兼容：`.opendomain/index.json`

`opendomain init` 只创建 canonical 工作区。若项目只有 legacy `domain/`，
`init` 会拒绝创建一个会遮蔽旧语料的新根目录；当前版本不包含自动迁移命令。

### 3.2 YAML front matter 支持边界
...
### 3.3 Runtime Schema 与安全语料

`opendomain validate` 会按以下顺序处理输入：

```text
安全解析并规范化 YAML front matter
  ↓
按 type 执行对应的 Draft 2020-12 Schema
  ↓
执行 evidence、review、引用、Candidate 和 lifecycle 语义校验
```

Bounded Context、Domain Concept、Business Rule、Lifecycle、Domain Event 和
Domain Candidate 都必须通过 `schemas/` 中对应的 Schema，才能进入 validated
`documents` corpus。Schema 会检查必填字段、值类型、数组与嵌套对象结构、枚举、
ID pattern 和 `date` format。诊断会稳定返回 `file`、`field`、`problem` 和
`fix`；JSON 模式下可以直接供 CI 或其他工具消费。

结构不合法的对象不会继续满足引用，也不会进入 Semantic Closure、index 或
Grounding Pack 的 `read_first`。如果 npm 包内的必需 Schema 缺失、损坏或无法
编译，Validator 会失败关闭并返回空语料，而不是跳过 Schema 后继续运行。

通过 Schema 只代表单个文档结构符合契约，不代表其中的业务知识已经成立。
跨文档引用、accepted / proposed 边界和 lifecycle 一致性仍由语义校验处理，
accepted knowledge 仍必须经过人类审查。OpenSpec-style `feature_spec` 当前继续
使用自己的适配与语义校验，不属于上述六类领域 source schema。
...
```bash
opendomain prepare --integration openspec openspec/changes/order-cancellation/spec.md
```

`prepare` 始终从当前项目工作区读取长期领域语义，因此命令应在 Feature
...
在项目根目录省略 source path 和 `--out` 时，`index build` 会从默认工作区构建
并写入 `opendomain/generated/index.json`；`index query` 省略 `--index` 时读取
同一路径。Legacy `domain/` 模式在 `0.x` 期间继续使用
`.opendomain/index.json`。显式 `--out` / `--index` 始终优先。
