...
test("packaged resources expose schemas, package metadata, and ERP files", async () => {
  ...
  const sourceMetadata = JSON.parse(await readFile(new URL("../package.json", import.meta.url), "utf8"));
  assert.equal(packageMetadata.version, sourceMetadata.version);
  ...
  assert.ok(installationContract.includes(`@echopath-labs/opendomain@${sourceMetadata.version}`));
